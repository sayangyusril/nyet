...
END:VCARD
 ...
 conn.fakeReply = (jid, text = '', fakeJid = conn.user.jid, fakeText = '', fakeGroupJid, options) => {
     ...
     * @param {Object} options 
     */
    conn.sendBut = async (jid, contentText, footer, buffer, buttons, quoted, options) => {
...
return await conn.sendMessage(jid, buttonMessage, {quoted})
   ...
   conn.send2Button = async(jid, content, footer, button1, row1, button2, row2, quoted) => {
	  const buttons = [
	   { buttonId: row1, buttonText: { displayText: button1 }, type: 1 },
...
return await conn.sendMessage(jid, buttonMessage, {quoted})
   ...
   conn.send3Button = async(jid, content, footer,button1, row1, button2, row2, button3, row3, quoted) => {
	  const buttons = [
	  { buttonId: row1, buttonText: { displayText: button1 }, type: 1 },
          { buttonId: row2, buttonText: { displayText: button2 }, type: 1 },
